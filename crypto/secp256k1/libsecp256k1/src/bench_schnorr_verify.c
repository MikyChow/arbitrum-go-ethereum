...
static void benchmark_schnorr_init(void* arg) {
    ...
    for (k = 0; k < data->numsigs; k++) {
        ...
        CHECK(arb_secp256k1_ec_pubkey_create(data->ctx, &pubkey, data->sigs[k].key));
        CHECK(arb_secp256k1_ec_pubkey_serialize(data->ctx, data->sigs[k].pubkey, &data->sigs[k].pubkeylen, &pubkey, SECP256K1_EC_COMPRESSED));
    }
}
...
static void benchmark_schnorr_verify(void* arg) {
    ...
    for (i = 0; i < 20000 / data->numsigs; i++) {
        ...
        CHECK(arb_secp256k1_ec_pubkey_parse(data->ctx, &pubkey, data->sigs[0].pubkey, data->sigs[0].pubkeylen));
