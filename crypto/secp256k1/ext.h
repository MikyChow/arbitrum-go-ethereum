...
// arb_secp256k1_context_create_sign_verify creates a context for signing and signature verification.
static secp256k1_context* arb_secp256k1_context_create_sign_verify() {
	return arb_secp256k1_context_create(SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY);
}
...
static int secp256k1_ext_ecdsa_recover(
	...
	if (!arb_secp256k1_ecdsa_recoverable_signature_parse_compact(ctx, &sig, sigdata, (int)sigdata[64])) {
		return 0;
	}
	if (!arb_secp256k1_ecdsa_recover(ctx, &pubkey, &sig, msgdata)) {
		return 0;
	}
	size_t outputlen = 65;
	return arb_secp256k1_ec_pubkey_serialize(ctx, pubkey_out, &outputlen, &pubkey, SECP256K1_EC_UNCOMPRESSED);
}
...
static int secp256k1_ext_ecdsa_verify(
	...
	if (!arb_arb_secp256k1_ecdsa_signature_parse_compact(ctx, &sig, sigdata)) {
		return 0;
	}
	if (!arb_secp256k1_ec_pubkey_parse(ctx, &pubkey, pubkeydata, pubkeylen)) {
		return 0;
	}
	return arb_secp256k1_ecdsa_verify(ctx, &sig, msgdata, &pubkey);
}
...
static int secp256k1_ext_reencode_pubkey(
	...
	return arb_secp256k1_ec_pubkey_serialize(ctx, out, &outlen, &pubkey, flag);
}

// arb_secp256k1_ext_scalar_mul multiplies a point by a scalar in constant time.
//
...
int arb_secp256k1_ext_scalar_mul(const secp256k1_context* ctx, unsigned char *point, const unsigned char *scalar) {
	int ret = 0;
	int overflow = 0;
